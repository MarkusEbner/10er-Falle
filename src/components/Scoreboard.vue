  ...
  <div>
    <v-card>
      ...
      <v-list v-if="players.length > 0">
        <v-card-subtitle v-if="currentPlayer"
          >Aktueller Spieler: {{ currentPlayer.name }}</v-card-subtitle
        >
        <v-list-item
    ...
    <v-card>
      ...
      <v-card-text v-if="(currentPlayer?.score ?? 0) > 6">
        <p>
          ...
          <v-icon icon="mdi-face-man" /> hat durch hochdrehen verloren!
        </p>
      </v-card-text>
      <v-card-text v-else>
        <p>
          ...
          <v-icon icon="mdi-face-man" /> hat verloren!
        </p>
        <p>
          Letzter Wurf: {{ sessionData?.last_roll }}
          <v-icon :icon="getDiceIcon(sessionData?.last_roll)" class="ml-2" />
        </p>
...
<script lang="ts" setup>
  ...
  const playerName = computed(
    () => players.value.find((player) => player.id === playerId.value)?.name
  );
  ...
  const rollLebenswürfel = async () => {
    ...
    const { error: sessionError } = await supabase
      .from('sessions')
      .update({
        current_player_id: players.value[0].id,
        current_score: players.value[0].score,
        player_order: players.value.map((player) => player.id),
      })
  ...
  const rollSpielwürfel = async () => {
    ...
    const allgemeinerSpielstand =
      sessionData.value.current_score + aktuellerWurf;
    console.log(
      `Aktueller Wurf: ${aktuellerWurf}. Neuer Spielstand: ${allgemeinerSpielstand}`
    );
    ...
    const { error } = await supabase
      .from('sessions')
      .update({
        current_score: allgemeinerSpielstand,
        last_roll: aktuellerWurf,
      })
      .eq('id', props.sessionId);
    if (error) {
  ...
  const gameOverAlert = async (
    aktuellerWurf?: number,
    allgemeinerSpielstand?: number
  ) => {
    ...
    if (aktuellerWurf && allgemeinerSpielstand) {
      ...
      if (error) {
        console.error(
          'Fehler beim Beenden des Spiels durch normales Würfeln:',
          error
        );
      }
    } else {
      ...
      if (error) {
        console.error(
          'Fehler beim Beenden des Spiels durch Hochdrehen:',
          error
        );
      }
    }
  ...
  const hochdrehen = async (erzwungen: boolean) => {
    ...
    if (error) {
      ...
      const { error: sessionError } = await supabase
        .from('sessions')
        .update({
          current_player_id: currentPlayer.value.id,
          current_score: currentPlayer.value.score,
        })
        .eq('id', props.sessionId);

      if (sessionError) {
        console.error(
          'Fehler beim Setzen des aktuellen Spielers:',
          sessionError
        );
        return;
      }
      console.log(
        `Spieler ${currentPlayer.value.name} hat hochgedreht. Er darf nochmal den Spielwürfel würfeln!`
      );
    }
  };
  ...
  const switchToNextPlayer = async () => {
    ...
    currentPlayerIndex.value = sessionData.value.player_order.indexOf(
      sessionData.value.current_player_id
    );
    const nextPlayerIndex =
      (currentPlayerIndex.value + 1) % sessionData.value.player_order.length;
      ...
      .on(
        ...
        (payload) => {
          ...
          if (payload.new) {
            ...
            if (
              currentPlayer.value &&
              currentPlayer.value.id === playerId.value
            ) {
              yourTurn.value = true;
            } else {
